            ...
            <div class="col-sm-4 col-12 filter-box">
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rType" id="mType" value="non-veg" hidden
                        ...
                        <li>
                            <input type="button" name="rType" id="vType" value="veg" hidden
                                @click="filterTypeBtn($event)" />
                ...
                <div class="row">
                    <div class="menu-tabs">
                        ...
                        <input type="button" id="dessertFilterFoodBtn" name="dessertFilterFoodBtn" class="menu-tab-item"
                            value="Starters" @click="filterFoodBtn($event)" />
                        <input type="button" id="drinkFilterFoodBtn" name="drinkFilterFoodBtn" class="menu-tab-item"
                            value="Desserts" @click="filterFoodBtn($event)" />
                        <input type="button" id="drinkFilterFoodBtn" name="drinksFilterFoodBtn" class="menu-tab-item"
...
export default {
    ...
    methods: {
        ...
        evaluatePrice: function (food, priceRange) {
            this.pageNum = 0;
            var cal = parseFloat(food.food_price);
            if (priceRange == "2,5") {
                if (2 <= cal && cal <= 5) {
                    return food;
                }
    ...
    this.pageNum = 0;
    // Toggle the type if clicking the same button again
    if (this.foodObj.type === e.target.value) {
        this.foodObj.type = "";
        document.querySelector(`[for=${e.target.id}]`).style.background = "inherit";
        document.querySelector(`[for=${e.target.id}]`).style.color = "inherit";
        document.querySelector(`[for=${e.target.id}]`).querySelector(":scope > button").style.display = "none";
        this.previousTypeClicked = "";
    } else {
        this.foodObj.type = e.target.value.toLowerCase(); // Ensure lowercase
        document.querySelector(`[for=${e.target.id}]`).style.background = "#057835fa";
        document.querySelector(`[for=${e.target.id}]`).style.color = "white";
        document.querySelector(`[for=${e.target.id}]`).querySelector(":scope > button").style.display = "block";
        if (this.previousTypeClicked && this.previousTypeClicked.target.id !== e.target.id) {
